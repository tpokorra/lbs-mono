...
ssh $TARGETUSER@$TARGETHOST "dpkg-query -f '\${Package}\n' -W" > pkgs_target.txt
...
apt-get install $MONO_PACKAGES --dry-run | grep -E "^Inst " | awk '{print $2}' > pkgs_mono.txt

# install the mono packages
apt-get -y install $MONO_PACKAGES
